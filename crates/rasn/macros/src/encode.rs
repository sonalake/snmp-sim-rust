...
pub fn map_to_inner_type(
    ...
    inner_generics
        .params
        .push(syn::LifetimeDef::new(syn::Lifetime::new("'inner", proc_macro2::Span::call_site())).into());
    ...
    let (field_defs, init_fields) = match &fields {
        syn::Fields::Named(_) => {
            ...
            fn wrap(fields: impl Iterator<Item = proc_macro2::TokenStream>) -> proc_macro2::TokenStream {
                quote!({ #(#fields),* })
            }
