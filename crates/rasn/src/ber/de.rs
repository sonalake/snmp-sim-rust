...
impl<'input> Decoder<'input> {
    ...
    pub(crate) fn parse_value(&mut self, tag: Tag) -> Result<(Identifier, Option<&'input [u8]>)> {
        let (input, (identifier, contents)) = self::parser::parse_value(&self.config, self.input, Some(tag))?;
        self.input = input;
...
impl<'input> crate::Decoder for Decoder<'input> {
    ...
    fn decode_any(&mut self) -> Result<types::Any> {
        let (mut input, (identifier, contents)) = self::parser::parse_value(&self.config, self.input, None)?;

        if contents.is_none() {
            let (i, _) = self::parser::parse_encoded_value(&self.config, self.input, identifier.tag, |input| {
    ...
    fn decode_integer(&mut self, tag: Tag) -> Result<types::Integer> {
        Ok(types::Integer::from_signed_bytes_be(self.parse_primitive_value(tag)?.1))
    }
            ...
            match contents {
                Some(mut contents) => {
                    while !contents.is_empty() {
                        let (c, mut vec) =
                            self::parser::parse_encoded_value(&self.config, contents, Tag::OCTET_STRING, |input| {
                                Ok(alloc::vec::Vec::from(input))
                            })?;
                        contents = c;
                ...
                None => {
                    while !self.input.starts_with(EOC) {
                        let (c, mut vec) =
                            self::parser::parse_encoded_value(&self.config, self.input, Tag::OCTET_STRING, |input| {
                                Ok(alloc::vec::Vec::from(input))
                            })?;
                        self.input = c;
    ...
    fn decode_object_identifier(&mut self, tag: Tag) -> Result<crate::types::ObjectIdentifier> {
        ...
        let (mut contents, root_octets) = parser::parse_base128_number(contents).map_err(error::map_nom_err)?;
    ...
    fn decode_bit_string(&mut self, tag: Tag) -> Result<types::BitString> {
        let (input, bs) = self::parser::parse_encoded_value(&self.config, self.input, tag, |input| {
    ...
    fn decode_sequence<D, F: FnOnce(&mut Self) -> Result<D>>(&mut self, tag: Tag, decode_fn: F) -> Result<D> {
...
mod tests {
    ...
    fn octet_string() {
        ...
        assert_eq!(octet_string, decode::<types::OctetString>(primitive_encoded).unwrap());
        assert_eq!(octet_string, decode::<types::OctetString>(constructed_encoded).unwrap());
    }

    #[test]
    fn bit_string() {
        let bitstring = types::BitString::from_vec([0x0A, 0x3B, 0x5F, 0x29, 0x1C, 0xD0][..].to_owned());
    ...
    fn sequence_of() {
        ...
        let from_raw: Vec<String> =
            decode(&[0x30, 0x9, 0x0C, 0x03, 0x4A, 0x6F, 0x6E, 0x0C, 0x02, 0x65, 0x73][..]).unwrap();
    ...
    fn flip1() {
        let _ = decode::<Open>(&[
            0x10, 0x10, 0x23, 0x00, 0xfe, 0x7f, 0x10, 0x03, 0x00, 0xff, 0xe4, 0x04, 0x50, 0x10, 0x50, 0x10, 0x10, 0x10,
        ]);
    }
    ...
    fn any_indefinite() {
        let any = &[
            0x30, 0x80, 0x2C, 0x80, 0x04, 0x03, 0x4A, 0x6F, 0x6E, 0x04, 0x02, 0x65, 0x73, 0x00, 0x00, 0x00, 0x00,
        ];
        assert_eq!(Any { contents: any.to_vec() }, decode(any).unwrap(),);
    }

    #[test]
    fn any_indefinite_fail_no_eoc() {
        let any = &[
            0x30, 0x80, 0x2C, 0x80, 0x04, 0x03, 0x4A, 0x6F, 0x6E, 0x04, 0x02, 0x65, 0x73, 0x00, 0x00,
        ];
    ...
    fn decoding_oid() {
        ...
        let mut decoder = super::Decoder::new(&[0x06, 0x03, 0x88, 0x37, 0x01], DecoderOptions::der());
