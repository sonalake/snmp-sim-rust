    ...
    RV: Appendable,
{
    ...
    if identifier.is_primitive() {
        ...
        while !input.is_empty() && !input.starts_with(EOC) {
            ...
            let (i, mut child) = parse_encoded_value(config, input, identifier.tag, primitive_callback)?;
            input = i;
...
mod tests {
    ...
    fn value_long_length_form() {
        let (_, (_, contents)) =
            parse_value(&BER_OPTIONS, [0x1, 0x81, 0x2, 0xF0, 0xF0][..].into(), Tag::BOOL.into()).unwrap();
