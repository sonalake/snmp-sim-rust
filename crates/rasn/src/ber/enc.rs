...
impl Encoder {
    ...
    /// ---------------------------------
    /// ```
    fn encode_identifier(&mut self, Identifier { tag, is_constructed }: Identifier) -> ByteOrBytes {
...
impl crate::Encoder for Encoder {
    ...
    fn encode_bit_string(&mut self, tag: Tag, value: &types::BitString) -> Result<Self::Ok, Self::Error> {
        if value.not_any() {
    ...
    fn encode_utc_time(&mut self, tag: Tag, value: &types::UtcTime) -> Result<Self::Ok, Self::Error> {
    ...
    fn encode_generalized_time(&mut self, tag: Tag, value: &types::GeneralizedTime) -> Result<Self::Ok, Self::Error> {
        self.encode_primitive(
            tag,
            value
                .naive_utc()
    ...
    fn encode_set_of<E: Encode>(&mut self, tag: Tag, values: &types::SetOf<E>) -> Result<Self::Ok, Self::Error> {
        let mut sequence_encoder = Self::new(self.config);
...
mod tests {
    ...
    fn identifier() {
        ...
        assert_eq!(
            &[0xFF, 0x7F,][..],
            ident_to_bytes(Identifier::from_tag(Tag::new(crate::types::Class::Private, 127), true,))
        );
    }
    ...
    fn encoding_oid() {
        ...
        assert_eq!(&vec![0x06, 0x03, 0x55, 0x04, 0x03], &oid_to_bytes(&[2, 5, 4, 3]));

        // example oid
        assert_eq!(&vec![0x06, 0x03, 0x88, 0x37, 0x01], &oid_to_bytes(&[2, 999, 1]));
    }
    ...
    fn set() {
        ...
        assert_eq!(vec![0x31, 0x9, 0x80, 0x1, 0x1, 0x81, 0x1, 0x2, 0x82, 0x1, 0x3], output,);
    }
}
