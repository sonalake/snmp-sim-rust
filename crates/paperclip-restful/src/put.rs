...
pub enum PutResponse<T: Serialize + Apiv2SchemaTrait> {
    Created(T),
    Updated(T),
}

impl<T: Serialize + Apiv2SchemaTrait> Responder for PutResponse<T> {
    type Body = BoxBody;

    fn respond_to(self, req: &HttpRequest) -> HttpResponse<Self::Body> {
        match self {
            Self::Created(c) => CreatedJson(c).respond_to(req).map_into_boxed_body(),
            Self::Updated(c) => Json(c).respond_to(req).map_into_boxed_body(),
        }
    }
}

impl<Schema: Serialize + Apiv2SchemaTrait> Apiv2SchemaTrait for PutResponse<Schema> {}

impl<Schema: Serialize + Apiv2SchemaTrait> OperationModifier for PutResponse<Schema> {
